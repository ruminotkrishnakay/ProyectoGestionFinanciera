...
S<- tail(ENELCHILE$close,1)
K<- 40 # Precio del ejercicio
R<- 0.0642 # Tasa libre de riesgo (Bonos BCH a 10 años al 12 de oct)
t <- 1
...
### Modelo alternativo Binomial

El Modelo Binomial de valoración de opciones (BM por sus siglas en inglés) también conocido como Cox-Ross-Rubinstein, es un modelo que estima el valor de una opción en t=0 teniendo pagos futuros en sitintos estados (usualmente "Up" y "Down"). Este Modelo Binomial asume que no hay posibilidad de arbitraje en el mercado, detrminando el precio que sigue un activo activo sigue un random walk, en donde cada paso (tiempo) existe una proabilidad de que el precio suba o baje. (Ahmad, 2018).

![Random Walk en BM de un paso (Ahmad, 2018)](Data/Screenshot%202023-10-19%20at%2010.33.38.png){fig-align="center"}

Para calcular la valoración de la opción con el BM se debe definir $S_0$ que corresponde al precio actual del activo. Asimismo se debe determinar $u$ y $d$, factores por el que se multiplica $S_0$ en cada estado para obtener el precio del activo en cada caso en el siguiente periodo (t = 1):

$$
S_1 = \begin{cases}    S_0*u \text{ en el estado "Up"} \\    
S_0*d \text{ en el estado "Down"}\end{cases}
$$

También se debe calcular la probabilidad $P$ de que ocurra el caso "Up", la porobabilidad de que ocurra el otro caso ("Down") simplemete quedaría en $(1-P)$. Pare el calculo de la porobabilidad se usa la siguiente ecuación:
...
print(binomial_model(cu, p, cd, r))

```

### Comparación de modelos

Ambos modelos son comunmente utilizados en las finanzas, por ende se puede afirmar que ambos modelos son resapaldados y tienen sustentos. Segun el paper (Ahmad, 2018) el resultado del precio de la opción calculado con ambos modelos tienen una correlación muy cercana a 1, lo que que significa que si con un modelo aumenta el precio de la opción, con el otro también. Sin embargo, esto no resulta en que ambos modelos nos otorgen el mismo precio, tal como se ve en el ejercicio anterior.
