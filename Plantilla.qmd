...
### Modelo alternativo: Modelo Binomial

El Modelo Binomial de Valoración de Opciones (BM, por sus siglas en inglés) también conocido como Cox-Ross-Rubinstein, es un modelo que estima el valor de una opción en t=0 teniendo pagos futuros en diferentes estados (usualmente "Up" y "Down"). Este Modelo Binomial asume que no hay posibilidad de arbitraje en el mercado, determinando el precio que sigue un activo en un random walk, en donde en cada paso (tiempo) existe una probabilidad de que el precio suba o baje (Ahmad, 2018).
...
![Datos ENELCHILE para Caso Cap.28](Data/Screenshot 2023-10-26 at 19.47.47.png)

A partir de lo anterior, se puede determianar que el la opción 3 es la mejor debido a que obtiene el VPN mayor.
